...
const DiscoverProvider = ({ children }) => {
  ...
  const orderByYears = (setYearEndPoint, chosenYear, interval, mediaAdvance) => {
    switch (interval) {
      case 'before':
  ...
  const dataSortBy = useFetch(
    ...
    orderBy !== "original_name.asc" && orderBy !== "original_name.desc" && `&sort_by=${orderBy}`}&page=${discoverPage}`,
    [yearEndPoint, areGenres, mediaAdvance, genresAdvance, orderBy, chosenYear, interval, discoverPage]
  );
  ...
  useEffect(() => {
    ...
    dataSortBy && orderByYears(setYearEndPoint, chosenYear, interval, mediaAdvance);
    dataSortBy && setDiscover(dataSortBy.results);
    dataSortBy && setDiscoverMaxPage(dataSortBy.total_pages);
    dataSortBy && orderByYears(setYearEndPoint, chosenYear, interval, mediaAdvance);
